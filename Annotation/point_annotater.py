...
while True:
    ...
    if key == ord('p'):  # Pause/Resume
        ...
        curr_time = frame_idx
        curr_time_str = frame_to_time(curr_time)
        label = input(f"Label for event at {curr_time_str}: ")
        annotations.append([video_path,curr_time, label])
        print(f"[Saved] {label}: {curr_time}")
