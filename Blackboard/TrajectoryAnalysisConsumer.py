...
class TrajectoryAnalysisConsumer(Consumer):
    ...
    def interpolate_missing_frames(self,segment_frames, segment_positions, valid_frames, valid_positions, max_gap=5):
        ...
        interp_x = interp1d(valid_frames, valid_positions_array[:, 0], kind='cubic', bounds_error=False, fill_value='extrapolate')
        interp_y = interp1d(valid_frames, valid_positions_array[:, 1], kind='cubic', bounds_error=False, fill_value='extrapolate')
    ...
    def smooth_trajectory_kalman(self,segment_frames, interpolated_positions, valid_positions):
    ...
    def correct_bounces_with_table(self,smoothed_positions, table_coords):
        ...
        top_y = min(table_coords["tabley1"], table_coords["tabley2"], 
                    table_coords["tabley3"], table_coords["tabley4"])
        bottom_y = max(table_coords["tabley1"], table_coords["tabley2"], 
                    table_coords["tabley3"], table_coords["tabley4"])
    ...
    def detect_bounce_points(self,smoothed_positions, table_coords, proximity_threshold=15, min_velocity_change=0.5, segment_frames=None):
        """
        ...
        if segment_frames is None or len(segment_frames) != len(y):
            raise ValueError("segment_frames must be provided and match the length of smoothed_positions.")

        for i in range(2, len(y)-2):
            proximity_top = abs(y[i] - top_y) < proximity_threshold
            proximity_bottom = abs(y[i] - bottom_y) < proximity_threshold
            if (y[i] < y[i-1]) and (y[i] < y[i+1]) and (proximity_top or proximity_bottom):
    ...
    def logicfunction(self, messagebody):
        ...
        interpolated_ball_positions = self.interpolate_missing_frames(segment_frames, segment_positions, valid_frames, valid_positions, max_gap=5)
        ...
        for i, frame_id in enumerate(segment_frames):
            ...
            if pos is not None:
                interpolated_ball_positions_dict[frame_id] = {"ballx": pos[0], "bally": pos[1]}
            else:
                interpolated_ball_positions_dict[frame_id] = {"ballx": None, "bally": None}

        smoothed_positions, confidence_scores = self.smooth_trajectory_kalman(segment_frames, interpolated_ball_positions, valid_positions)
        ...
        self.saveresult(interpolated_ball_positions_dict, ball_velocities, bounceframes, messagebody["videoid"])
