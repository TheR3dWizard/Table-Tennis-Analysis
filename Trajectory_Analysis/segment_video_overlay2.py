...
def plot_segment_trajectory(segment_frames, valid_positions, valid_frames, smoothed_positions, 
    ...
    plt.subplot(2, 3, 1)
    # Removed lightblue raw detections scatter plot
    ...
    # Removed lightblue raw X scatter plot
...
def overlay_trajectory_on_video_segment(video_path, output_path, smoothed_positions, start_frame, end_frame, table_coords=None):
    ...
    # Prepare points for polylines (only smoothed trajectory)
    points = smoothed_positions.astype(np.int32)
...
def main():
    ...
    # Table coordinates print removed
...
def detect_bounce_points(smoothed_positions, table_coords, proximity_threshold=15, min_velocity_change=0.5):
    """
    Detect bounce points in the trajectory based on vertical velocity sign changes and table proximity.
    
    Args:
        smoothed_positions (np.ndarray): Array of shape (n, 2) with smoothed [x, y] coordinates.
        table_coords (tuple): Table coordinates (x1, y1, x2, y2) for top-left and bottom-right corners.
        proximity_threshold (float): Maximum distance (pixels) from table edges to consider a bounce.
        min_velocity_change (float): Minimum velocity change to filter out noise.
    
    Returns:
        list: List of tuples (frame_index, y_position, edge) where edge is 'top' or 'bottom'.
    """
    print("Detecting bounce points in trajectory...")
    
    if len(smoothed_positions) < 3:
        print("Too few positions to detect bounces")
        return []
    ...
    top_y = table_coords[1]
    bottom_y = table_coords[3]
    print(f"Table boundaries: top_y={top_y}, bottom_y={bottom_y}")
    
    # Detect bounces by checking velocity sign changes
    for i in range(1, len(smoothed_positions) - 1):
        # Check for velocity sign change
        if vy[i-1] * vy[i+1] < 0:
            # Ensure significant velocity change to avoid noise
            velocity_change = abs(vy[i-1] - vy[i+1])
            if velocity_change < min_velocity_change:
                continue
                
            # Check proximity to table edges
            y_pos = smoothed_positions[i, 1]
            if abs(y_pos - top_y) < proximity_threshold:
                bounce_points.append((i, y_pos, 'top'))
                print(f"Bounce detected at frame {i}, y={y_pos:.2f} (top edge)")
            elif abs(y_pos - bottom_y) < proximity_threshold:
                bounce_points.append((i, y_pos, 'bottom'))
                print(f"Bounce detected at frame {i}, y={y_pos:.2f} (bottom edge)")
    
    print(f"Detected {len(bounce_points)} bounce points")
    return bounce_points

def analyze_interpolation_error(interpolated_positions, smoothed_positions, segment_frames, segment_label="s1", save_plot=True):
    """
    Quantify interpolation accuracy by comparing interpolated positions to smoothed positions.

    Args:
        interpolated_positions (list): List of [x, y] positions (or None) from interpolate_missing_frames.
        smoothed_positions (np.ndarray): Array of shape (n, 2) with smoothed [x, y] coordinates.
        segment_frames (np.ndarray): Array of frame numbers for the segment.
        segment_label (str): Label for the segment (e.g., 's1').
        save_plot (bool): Whether to save a visualization of the interpolation error.

    Returns:
        dict: Dictionary containing interpolation error analysis results:
            - avg_error: Average Euclidean distance (pixels) between interpolated and smoothed positions.
            - max_error: Maximum Euclidean distance (pixels).
            - error_list: List of Euclidean distances for valid frames.
            - valid_frame_indices: List of frame indices where errors were calculated.
    """
    print(f"\n🏓 Analyzing Interpolation Error for Segment {segment_label}")
    print("=" * 60)
    ...
    errors = []
    valid_indices = []
    for i, (interp_pos, smooth_pos) in enumerate(zip(interpolated_positions, smoothed_positions)):
        if interp_pos is not None:
            error = np.sqrt((interp_pos[0] - smooth_pos[0])**2 + (interp_pos[1] - smooth_pos[1])**2)
            errors.append(error)
            valid_indices.append(i)
            print(f"Frame {segment_frames[i]}: Interpolation error = {error:.2f} pixels")

    if not errors:
        print("No valid interpolated positions to compare, skipping error analysis")
        return results

    results['error_list'] = errors
    results['valid_frame_indices'] = valid_indices
    results['avg_error'] = np.mean(errors)
    results['max_error'] = np.max(errors)
    print(f"Average interpolation error: {results['avg_error']:.2f} pixels")
    print(f"Maximum interpolation error: {results['max_error']:.2f} pixels")
    print(f"Number of frames compared: {len(errors)}")
    ...
    if save_plot:
        ...
        plt.tight_layout()
        os.makedirs("trajectory_segments", exist_ok=True)
        filename = f"trajectory_segments/interpolation_error_{segment_label}.png"
        plt.savefig(filename, dpi=150, bbox_inches='tight')
        print(f"Interpolation error plot saved as: {filename}")
        plt.close()
        print("Closed interpolation error plot")

    print("Interpolation error analysis completed")
    return results